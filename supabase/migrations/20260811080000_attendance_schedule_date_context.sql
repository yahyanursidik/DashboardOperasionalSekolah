-- Attendance must follow the period containing the work date, not the period
-- currently selected in an administrator's navigation context.

create or replace function public.employee_schedule_applies_on(
    ...
    (
      ...
      or exists (
        ...
        where academic_year.id = p_academic_year_id
          and (academic_year.start_date is null or academic_year.start_date <= p_work_date)
          and (academic_year.end_date is null or p_work_date <= academic_year.end_date)
      )
    )
    and (
      ...
      or exists (
        ...
        where semester.id = p_semester_id
          and (semester.start_date is null or semester.start_date <= p_work_date)
          and (semester.end_date is null or p_work_date <= semester.end_date)
      )
    );
$$;
...
comment on function public.resolve_employee_attendance_rule(uuid, date) is
  'Resolves attendance timing from schedules valid on the requested work date, independent of UI academic-period selection.';
