...
export const SpmbPayment: React.FC = () => {
  ...
  const submit = async (file?: File) => {
    ...
    try {
      ...
      const { data: savedPayment, error } = await db.from("admission_payments").upsert({ applicant_id: applicant.id, payment_type: "registration", amount: Number(amount), paid_at: paidAt, proof_url: uploaded.filePath, status: "submitted", verification_note: null, verified_at: null, verified_by: null }, { onConflict: "applicant_id,payment_type" }).select("*").single();
      if (error) throw error;
