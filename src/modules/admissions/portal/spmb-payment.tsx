...
import { ADMISSION_FILE_ACCEPT, ADMISSION_PAYMENT_PROOF_MAX_BYTES, admissionUploadContentType, admissionUploadError } from "../admission-upload";
...
export const SpmbPayment: React.FC = () => {
  ...
  const submit = async (file?: File) => {
    ...
    const validationError = admissionUploadError(file, ADMISSION_PAYMENT_PROOF_MAX_BYTES);
    if (validationError) { toast.error(validationError); return; }
    setLoading(true);
    let uploadedPath: string | null = null;
    try {
      const uploaded = await uploadDocument(file, `admissions/${applicant.id}/payments`, admissionUploadContentType(file));
