...
import { ADMISSION_DOCUMENT_MAX_BYTES, ADMISSION_FILE_ACCEPT, admissionUploadContentType, admissionUploadError } from "../admission-upload";
import { getCommitmentLetterKind, getCommitmentLetterTemplate } from "../commitment-letter-templates";
import { AdmissionCommitmentLetterExample } from "../components/admission-commitment-letter-example";
...
export const SpmbDocuments: React.FC = () => {
  ...
  const upload = async (type: string, file?: File) => {
    ...
    const validationError = admissionUploadError(file, ADMISSION_DOCUMENT_MAX_BYTES);
    if (validationError) { toast.error(validationError); return; }
    setUploading(type);
    let uploadedPath: string | null = null;
    try {
      const uploaded = await uploadDocument(file, `admissions/${applicant.id}/documents`, admissionUploadContentType(file));
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div><p className="text-sm font-semibold text-emerald-700">Tahap 2 dari 4 · {applicant.name}</p><h1 className="text-2xl sm:text-3xl font-bold">Berkas Persyaratan</h1><p className="text-slate-600 mt-2">Unggah seluruh berkas wajib terlebih dahulu. PDF, JPG, atau PNG maksimal 5 MB. Mengganti berkas akan mengulang pemeriksaan panitia.</p></div>
      ...
      <div className="space-y-3">{admissionDocumentTypes.map((type) => { const required = isRequiredAdmissionDocument(type.value, applicant); const doc = documents.find((item) => item.document_type === type.value); const isValid = doc?.status === "valid"; const needsRevision = ["revision_required", "rejected"].includes(doc?.status); const tone = isValid ? "border-emerald-200 bg-emerald-50/50" : needsRevision ? "border-rose-200 bg-rose-50/50" : "bg-white"; const isCommitmentLetter = type.value === "home_learning_commitment"; const label = isCommitmentLetter ? getCommitmentLetterTemplate(getCommitmentLetterKind(applicant)).title : type.label; return <div key={type.value} className={`border rounded-lg p-4 sm:p-5 ${tone}`}><div className="flex flex-col sm:flex-row sm:items-center gap-4"><div className={`w-10 h-10 rounded-md grid place-items-center shrink-0 ${isValid ? "bg-emerald-100 text-emerald-700" : needsRevision ? "bg-rose-100 text-rose-700" : "bg-slate-100 text-slate-600"}`}>{isValid ? <CheckCircle2 className="w-5 h-5" /> : needsRevision ? <AlertCircle className="w-5 h-5" /> : <UploadCloud className="w-5 h-5" />}</div><div className="min-w-0 flex-1"><p className="font-bold">{label} {required && <span className="text-rose-600">*</span>}</p><p className="text-sm text-slate-600 truncate mt-1">{doc ? `${doc.file_name} · ${isValid ? "Valid" : needsRevision ? "Perlu diperbaiki" : "Menunggu pemeriksaan"}` : "Belum diunggah"}</p>{doc?.review_note && <p className="text-sm text-rose-700 mt-2">Catatan panitia: {doc.review_note}</p>}</div><div className="flex gap-2">{doc && <button onClick={() => openFile(doc.file_url)} title="Lihat berkas" className="w-10 h-10 grid place-items-center border rounded-md hover:bg-white"><ExternalLink className="w-4 h-4" /></button>}<label className="h-10 px-4 border rounded-md bg-white hover:bg-slate-50 font-semibold text-sm flex items-center gap-2 cursor-pointer">{uploading === type.value ? <Loader2 className="w-4 h-4 animate-spin" /> : <UploadCloud className="w-4 h-4" />}{doc ? "Ganti" : "Unggah"}<input type="file" className="sr-only" accept={ADMISSION_FILE_ACCEPT} disabled={Boolean(uploading)} onChange={(event) => { void upload(type.value, event.target.files?.[0]); event.target.value = ""; }} /></label></div></div>{isCommitmentLetter && <AdmissionCommitmentLetterExample applicant={applicant} />}</div>; })}</div>
