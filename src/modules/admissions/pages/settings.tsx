...
import { AlertTriangle, CheckCircle2, Copy, Gauge, Loader2, Megaphone, Pencil, Plus, RotateCcw, Save, Settings2, UsersRound } from "lucide-react";
...
export const AdmissionsSettings: React.FC = () => {
  ...
  const [announcement, setAnnouncement] = useState({ title: "", body: "", batch_id: "", priority: "normal", publish_now: true });
  const [savingAnnouncement, setSavingAnnouncement] = useState(false);

  const load = async () => {
    setLoading(true);
    const [u, y, c, b, q, a, fees, news] = await Promise.all([
      ...
      db.from("admission_announcements").select("*,units(name),academic_years(name),admission_batches(name)").order("published_at", { ascending: false }),
    ]);
    setUnits(u.data || []); setYears(y.data || []); setClasses(c.data || []); setBatches(b.data || []); setPlans(q.data || []); setApplicants(a.data || []); setFeeRules(fees.data || []); setAnnouncements(news.data || []);
  ...
  const saveAnnouncement = async () => {
    if (!form.unit_id || !form.academic_year_id || !announcement.title.trim() || !announcement.body.trim()) { toast.error("Pilih unit dan tahun ajaran pada editor, lalu isi judul serta isi pengumuman."); return; }
    setSavingAnnouncement(true);
    const { error } = await db.from("admission_announcements").insert({ unit_id: form.unit_id, academic_year_id: form.academic_year_id, batch_id: announcement.batch_id || null, title: announcement.title.trim(), body: announcement.body.trim(), priority: announcement.priority, status: announcement.publish_now ? "published" : "draft", published_at: announcement.publish_now ? new Date().toISOString() : null });
    setSavingAnnouncement(false);
    if (error) toast.error(error.message); else { toast.success(announcement.publish_now ? "Pengumuman diterbitkan ke portal orang tua." : "Pengumuman disimpan sebagai draf."); setAnnouncement({ title: "", body: "", batch_id: "", priority: "normal", publish_now: true }); await load(); }
  };
  const updateAnnouncementStatus = async (id: string, status: string) => { const { error } = await db.from("admission_announcements").update({ status, published_at: status === "published" ? new Date().toISOString() : null }).eq("id", id); if (error) toast.error(error.message); else { toast.success(status === "published" ? "Pengumuman diterbitkan." : "Pengumuman disembunyikan."); await load(); } };
  ...
  return <div className="space-y-6">
    ...
    <section className="bg-white border rounded-lg overflow-hidden">
      <div className="p-5 sm:p-6 border-b"><SectionTitle icon={Megaphone} title="Pengumuman untuk Portal Orang Tua" detail="Tulis informasi per unit/tahun ajaran, atau batasi untuk satu gelombang." /></div>
      <div className="grid lg:grid-cols-2">
        <div className="p-5 sm:p-6 border-b lg:border-b-0 lg:border-r space-y-4">
          <p className="text-sm text-slate-600">Pilih unit dan tahun ajaran di editor gelombang di atas terlebih dahulu.</p>
          <Field label="Gelombang (opsional)"><select className={input} value={announcement.batch_id} onChange={(e) => setAnnouncement((v) => ({ ...v, batch_id: e.target.value }))}><option value="">Semua gelombang pada unit/tahun ini</option>{batches.filter((row) => row.unit_id === form.unit_id && row.academic_year_id === form.academic_year_id).map((row) => <option key={row.id} value={row.id}>{row.name}</option>)}</select></Field>
          <Field label="Judul *"><input className={input} value={announcement.title} onChange={(e) => setAnnouncement((v) => ({ ...v, title: e.target.value }))} placeholder="Contoh: Jadwal observasi dan persiapan" /></Field>
          <Field label="Isi pengumuman *"><textarea className="w-full min-h-32 mt-2 p-3 border rounded-md" value={announcement.body} onChange={(e) => setAnnouncement((v) => ({ ...v, body: e.target.value }))} placeholder="Sampaikan informasi yang diperlukan orang tua." /></Field>
          <div className="grid sm:grid-cols-2 gap-3">
            <Field label="Prioritas"><select className={input} value={announcement.priority} onChange={(e) => setAnnouncement((v) => ({ ...v, priority: e.target.value }))}><option value="normal">Informasi biasa</option><option value="important">Penting</option></select></Field>
            <label className="mt-7 inline-flex gap-2 text-sm font-semibold"><input type="checkbox" checked={announcement.publish_now} onChange={(e) => setAnnouncement((v) => ({ ...v, publish_now: e.target.checked }))} />Terbitkan sekarang</label>
          </div>
          <button onClick={saveAnnouncement} disabled={savingAnnouncement} className="h-10 px-4 bg-emerald-700 text-white rounded-md font-semibold text-sm inline-flex items-center gap-2 disabled:opacity-50">{savingAnnouncement ? <Loader2 className="w-4 h-4 animate-spin" /> : <Megaphone className="w-4 h-4" />}Simpan Pengumuman</button>
        </div>
        <div className="divide-y">
          {announcements.map((item) => <div key={item.id} className="p-5"><div className="flex gap-3"><Megaphone className="w-5 h-5 mt-0.5 text-emerald-700" /><div className="min-w-0 flex-1"><div className="flex flex-wrap gap-2 items-center"><p className="font-bold">{item.title}</p><span className={`px-2 py-0.5 rounded-full text-xs font-semibold ${item.status === "published" ? "bg-emerald-100 text-emerald-800" : "bg-slate-100 text-slate-700"}`}>{item.status === "published" ? "Terbit" : "Draf"}</span></div><p className="mt-2 text-sm text-slate-600 whitespace-pre-wrap">{item.body}</p><p className="mt-2 text-xs text-slate-500">{item.units?.name} · {item.academic_years?.name}{item.admission_batches?.name ? ` · ${item.admission_batches.name}` : ""}</p><button onClick={() => updateAnnouncementStatus(item.id, item.status === "published" ? "draft" : "published")} className="mt-3 h-8 rounded-md border px-3 text-xs font-semibold text-emerald-800">{item.status === "published" ? "Sembunyikan" : "Terbitkan"}</button></div></div></div>)}
          {announcements.length === 0 && <p className="p-8 text-center text-sm text-slate-500">Belum ada pengumuman yang dibuat.</p>}
        </div>
      </div>
    </section>
