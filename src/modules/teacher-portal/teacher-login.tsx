...
export const TeacherLogin: React.FC = () => {
  ...
  const handleLogin = async (event: React.FormEvent) => {
    ...
    try {
      ...
      toast.success("Berhasil masuk ke Portal Pengajar.");
  ...
  return (
    <div
      className="min-h-screen bg-emerald-950 px-4 py-8 sm:flex sm:items-center sm:justify-center"
    ...
    >
      <div className="mx-auto grid w-full max-w-4xl overflow-hidden rounded-lg border border-white/15 bg-white shadow-2xl md:grid-cols-[.85fr_1.15fr]">
        <section className="hidden bg-emerald-800 p-10 text-white md:flex md:flex-col md:justify-between">
          <div>
            {logoUrl ? (
              <img src={logoUrl} alt={appName || "Logo sekolah"} className="h-16 max-w-40 rounded-md bg-white p-2 object-contain" />
            ) : (
              <div className="flex h-14 w-14 items-center justify-center rounded-md bg-white text-emerald-800"><BookOpen className="h-7 w-7" /></div>
            )}
            <h1 className="mt-8 text-3xl font-bold">Portal Pengajar</h1>
            <p className="mt-3 max-w-xs text-sm leading-6 text-emerald-100">Akses jadwal, kelas, penilaian, jurnal, absensi, dan administrasi kerja dalam satu ruang.</p>
          </div>
          <p className="text-xs text-emerald-200">{appName || "Sistem Informasi Sekolah"}</p>
        </section>

        <section className="min-w-0 p-6 sm:p-10">
          <div className="mb-8 flex items-center gap-3 md:hidden">
            {logoUrl ? <img src={logoUrl} alt="Logo sekolah" className="h-11 w-11 rounded-md border object-contain p-1" /> : <BookOpen className="h-8 w-8 text-emerald-700" />}
            <div><p className="font-bold text-gray-900">Portal Pengajar</p><p className="text-xs text-gray-500">{appName || "Sistem Informasi Sekolah"}</p></div>
          </div>

          <h2 className="text-2xl font-bold text-gray-950">Masuk ke akun Anda</h2>
          <p className="mt-2 text-sm text-gray-500">Gunakan NIK atau email resmi dan kata sandi pribadi.</p>

          <form onSubmit={handleLogin} className="min-w-0 space-y-5 mt-8">
            <div>
              <label htmlFor="teacher-identifier" className="mb-1.5 block text-sm font-semibold text-gray-700">NIK / Email</label>
              <div className="relative min-w-0">
                ...
                <input id="teacher-identifier" autoComplete="username" required value={identifier} onChange={(event) => setIdentifier(event.target.value)} placeholder="NIK atau email resmi" className="h-12 w-full rounded-md border border-gray-300 bg-white pl-10 pr-3 text-sm outline-none focus:border-emerald-600 focus:ring-2 focus:ring-emerald-100" />
              </div>
            </div>

            <div>
              <label htmlFor="teacher-password" className="mb-1.5 block text-sm font-semibold text-gray-700">Kata Sandi</label>
              <div className="relative min-w-0">
                <LockKeyhole className="absolute left-3 top-3.5 h-5 w-5 text-gray-400" />
                <input id="teacher-password" type={showPassword ? "text" : "password"} autoComplete="current-password" required value={password} onChange={(event) => setPassword(event.target.value)} placeholder="Masukkan kata sandi" className="h-12 w-full rounded-md border border-gray-300 bg-white pl-10 pr-11 text-sm outline-none focus:border-emerald-600 focus:ring-2 focus:ring-emerald-100" />
            ...
            <button type="submit" disabled={isLoading || !identifier.trim() || !password} className="flex h-12 w-full items-center justify-center gap-2 rounded-md bg-emerald-700 px-4 text-sm font-bold text-white hover:bg-emerald-800 disabled:cursor-not-allowed disabled:opacity-60">
