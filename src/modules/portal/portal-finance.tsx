...
import { Receipt, CheckCircle, Clock, Upload, X, History, XCircle } from "lucide-react";
    ...
    <div className="p-4 sm:p-6 space-y-6">
      <div className="bg-gradient-to-r from-emerald-600 to-teal-600 rounded-2xl p-6 text-white shadow-lg relative overflow-hidden">
        ...
        <div className="relative z-10">
          ...
          <p className="text-emerald-50 text-sm">Pantau tagihan dan riwayat pembayaran Ananda di sini.</p>
        </div>
      </div>
      ...
      {activeTab === 'history' && (
        <div className="space-y-4">
          {invoices.flatMap(inv => inv.payment_transactions?.map((tx: any) => ({...tx, invoiceTitle: inv.title})) || []).length === 0 ? (
            <div className="bg-gray-50 rounded-xl border border-dashed p-8 text-center text-gray-500">
              <History className="w-12 h-12 mx-auto mb-3 opacity-20" />
              <p>Belum ada transaksi pembayaran.</p>
            </div>
          ) : (
            invoices.flatMap(inv => inv.payment_transactions?.map((tx: any) => ({...tx, invoiceTitle: inv.title})) || [])
