  ...
  return (
    <section className="space-y-5 rounded-lg border bg-card p-5 shadow-sm">
      <div className="flex flex-col gap-2 border-b pb-4 sm:flex-row sm:items-start sm:justify-between">
        <div><h2 className="flex items-center gap-2 font-bold"><Clock3 className="h-5 w-5 text-primary" /> Shift Peran dan Pegawai</h2><p className="mt-1 text-xs text-muted-foreground">Buat template jam kerja per jabatan, lalu tetapkan shift yang tepat kepada setiap pegawai.</p></div>
      ...
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_360px]">
        <form onSubmit={saveShift} className="space-y-4">
          <div className="flex items-center justify-between"><div><h3 className="text-sm font-bold">{form.id ? "Ubah template shift" : "Template shift baru"}</h3><p className="text-xs text-muted-foreground">Jam ini menjadi acuan terlambat dan pulang awal.</p></div>{form.id ? <button type="button" onClick={() => setForm(EMPTY_SHIFT)} className="text-xs font-bold text-muted-foreground">Batal ubah</button> : null}</div>
        ...
        <form onSubmit={assignShift} className="h-fit space-y-4 rounded-lg border bg-muted/20 p-4">
          <div><h3 className="flex items-center gap-2 text-sm font-bold"><UserRoundCheck className="h-4 w-4 text-primary" /> Tugaskan kepada pegawai</h3><p className="mt-1 text-xs text-muted-foreground">Daftar pegawai otomatis dibatasi sesuai jabatan dan unit shift.</p></div>
