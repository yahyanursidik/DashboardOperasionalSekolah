...
import { ArrowLeft, CalendarCheck, Clock, LogIn, LogOut, ShieldCheck } from "lucide-react";
...
export const StaffAttendance: React.FC = () => {
  ...
  const fetchData = async () => {
    setIsLoading(true);
    try {
      const [{ data: attendanceRows }, { data: leaveRows }] = await Promise.all([
        supabaseClient
  ...
  const saveAttendance = async (values: Record<string, any>, successMessage: string) => {
    ...
    try {
      const { error } = await supabaseClient
        .from("employee_attendance")
        .upsert({ employee_id: employee.id, date: today, ...values }, { onConflict: "employee_id,date" });
  ...
  const handleCheckIn = () => {
    saveAttendance(
      { status: record?.status || "present", time_in: record?.time_in || nowTime(), notes: record?.notes || "Absen mandiri dari portal staf." },
  ...
  const handleCheckOut = () => {
    ...
    saveAttendance(
      { status: record.status || "present", time_in: record.time_in, time_out: nowTime(), notes: record.notes || "Absen mandiri dari portal staf." },
  ...
  return (
    <div className="p-4 space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h2 className="font-black text-lg text-gray-900">Absensi Saya</h2>
      ...
      <section className="rounded-3xl bg-white border shadow-sm p-5">
        <div className="flex items-start justify-between gap-4">
          ...
          <span className={`rounded-full border px-3 py-1 text-xs font-black ${statusClass(record?.status)}`}>
        ...
        {activeLeave && (
          <div className="mt-4 rounded-2xl border border-blue-200 bg-blue-50 p-4 text-sm text-blue-800">
            <div className="flex items-start gap-2">
              ...
              <p>Izin Anda sudah disetujui untuk hari ini. Jika tetap hadir, simpan absen agar HRD mengetahui kondisi aktual.</p>
            </div>
          </div>
        )}

        <div className="mt-5 grid grid-cols-2 gap-3">
          <button onClick={handleCheckIn} disabled={isSaving} className="flex items-center justify-center gap-2 rounded-2xl bg-emerald-600 px-4 py-3 text-sm font-black text-white shadow-sm disabled:opacity-50">
            <LogIn className="w-4 h-4" /> {record?.time_in ? "Perbarui Masuk" : "Absen Masuk"}
          </button>
          <button onClick={handleCheckOut} disabled={isSaving || !record?.time_in} className="flex items-center justify-center gap-2 rounded-2xl border bg-white px-4 py-3 text-sm font-black text-gray-700 shadow-sm disabled:opacity-50">
          ...
          <div className="divide-y">
            {history.map((item) => (
              <div key={item.id} className="flex items-center justify-between py-3">
                <div>
                  <p className="text-sm font-black text-gray-900">{new Date(`${item.date}T00:00:00`).toLocaleDateString("id-ID", { weekday: "short", day: "2-digit", month: "short" })}</p>
