...
export const StaffOperationalReports: React.FC = () => {
  ...
  return <div className="space-y-5">
    <PageHeader title="Laporan Operasional" description="Laporkan kejadian, kerusakan, kebutuhan, atau serah terima agar dapat ditindaklanjuti dan dilacak." action={<button onClick={() => setIsFormOpen(true)} className="flex items-center justify-center gap-2 rounded-md bg-emerald-700 px-4 py-2.5 text-sm font-bold text-white hover:bg-emerald-800"><Plus className="h-4 w-4" />Buat Laporan</button>} />
    ...
    {isFormOpen && <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/50 p-0 sm:items-center sm:p-4"><form onSubmit={submitReport} className="max-h-[92vh] w-full max-w-xl overflow-y-auto rounded-t-lg bg-white p-5 text-slate-900 shadow-xl sm:rounded-lg"><div className="mb-5 flex items-start justify-between"><div><h2 className="font-bold text-gray-950">Buat Laporan Operasional</h2><p className="text-xs text-gray-500">Tuliskan fakta, lokasi, dan waktu kejadian secara jelas.</p></div><button type="button" onClick={() => setIsFormOpen(false)} title="Tutup" className="flex h-9 w-9 items-center justify-center rounded-md bg-gray-100 text-gray-700 hover:bg-gray-200"><X className="h-5 w-5" /></button></div><div className="grid gap-4 sm:grid-cols-2"><label className="text-xs font-bold text-gray-700">Kategori<select value={form.category} onChange={(event) => setForm({ ...form, category: event.target.value })} className="mt-1 h-10 w-full rounded-md border px-3 text-sm font-normal">{operationalReportCategories.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select></label><label className="text-xs font-bold text-gray-700">Prioritas<select value={form.priority} onChange={(event) => setForm({ ...form, priority: event.target.value })} className="mt-1 h-10 w-full rounded-md border px-3 text-sm font-normal"><option value="low">Rendah</option><option value="medium">Normal</option><option value="high">Tinggi</option><option value="urgent">Darurat</option></select></label><label className="text-xs font-bold text-gray-700">Unit<select value={form.unit_id} onChange={(event) => setForm({ ...form, unit_id: event.target.value })} className="mt-1 h-10 w-full rounded-md border px-3 text-sm font-normal"><option value="">Lintas unit</option>{units.map((unit) => <option key={unit.id} value={unit.id}>{unit.name}</option>)}</select></label><label className="text-xs font-bold text-gray-700">Waktu kejadian<input type="datetime-local" required value={form.occurred_at} onChange={(event) => setForm({ ...form, occurred_at: event.target.value })} className="mt-1 h-10 w-full rounded-md border px-3 text-sm font-normal" /></label><label className="text-xs font-bold text-gray-700 sm:col-span-2">Judul<input required maxLength={150} value={form.title} onChange={(event) => setForm({ ...form, title: event.target.value })} placeholder="Ringkasan kejadian atau kebutuhan" className="mt-1 h-10 w-full rounded-md border px-3 text-sm font-normal" /></label><label className="text-xs font-bold text-gray-700 sm:col-span-2">Lokasi<input value={form.location} onChange={(event) => setForm({ ...form, location: event.target.value })} placeholder="Contoh: Gerbang utama, toilet lantai 2" className="mt-1 h-10 w-full rounded-md border px-3 text-sm font-normal" /></label><label className="text-xs font-bold text-gray-700 sm:col-span-2">Uraian<textarea required rows={4} value={form.description} onChange={(event) => setForm({ ...form, description: event.target.value })} placeholder="Jelaskan kondisi, dampak, dan tindakan awal yang sudah dilakukan." className="mt-1 w-full rounded-md border p-3 text-sm font-normal" /></label></div><div className="mt-5 flex justify-end"><button disabled={isSaving} className="flex items-center gap-2 rounded-md bg-emerald-700 px-4 py-2.5 text-sm font-bold text-white hover:bg-emerald-800 disabled:cursor-not-allowed disabled:bg-slate-200 disabled:text-slate-600 disabled:opacity-100">{isSaving ? <Loader2 className="h-4 w-4 animate-spin" /> : <Send className="h-4 w-4" />}Kirim Laporan</button></div></form></div>}
  </div>;
};
