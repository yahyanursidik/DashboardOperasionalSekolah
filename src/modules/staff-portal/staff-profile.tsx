...
import { getAttendanceMode } from "../employees/employee-role-config";
...
export const StaffProfile: React.FC = () => {
  ...
  const updatePassword = async (event: React.FormEvent) => {
    ...
    try {
      ...
      setPassword(""); setConfirmPassword(""); setMustChangePassword(false);
  ...
  return (
    <div className="space-y-6">
      ...
      <PageHeader title="Profil Staf" description="Identitas, jadwal kerja, dan kesiapan data operasional." />

      <section className="overflow-hidden rounded-md border bg-white shadow-sm">
        <div className="h-20 border-b border-emerald-200 bg-emerald-100" />
        <div className="px-5 pb-5">
          <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
            <div className="flex min-w-0 flex-col gap-3 sm:flex-row sm:items-start">
              <div className="-mt-8 flex h-20 w-20 shrink-0 items-center justify-center rounded-md border-4 border-white bg-emerald-600 text-2xl font-black text-white shadow-sm">
      ...
      <section className="grid gap-5 lg:grid-cols-[1fr_1fr]">
        <div className="rounded-md border bg-card p-5">
          ...
          <InfoRow icon={Clock} label="Pola Absensi" value={getAttendanceMode(profile?.attendance_mode).label} />
        ...
        <div className="rounded-md border bg-card p-5">
          ...
          <div className="space-y-2">
            {checklist.map((item) => (
              <div key={item.label} className="flex items-center justify-between rounded-md border bg-muted/40 px-3 py-2">
