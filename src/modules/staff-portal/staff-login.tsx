...
export const StaffLogin: React.FC = () => {
  ...
  return (
    <div className="staff-light-theme min-h-screen bg-slate-50 px-4 py-8 text-slate-900 sm:flex sm:items-center sm:justify-center" style={loginCoverUrl ? { backgroundImage: `linear-gradient(rgba(248, 250, 252, .92), rgba(248, 250, 252, .97)), url(${loginCoverUrl})`, backgroundPosition: "center", backgroundSize: "cover" } : undefined}>
      <div className="mx-auto grid w-full max-w-4xl overflow-hidden rounded-lg border border-slate-200 bg-white shadow-lg md:grid-cols-[.85fr_1.15fr]">
        <section className="hidden border-r border-emerald-200 bg-emerald-50 p-10 text-slate-900 md:flex md:flex-col md:justify-between"><div>{logoUrl ? <img src={logoUrl} alt={appName || "Logo sekolah"} className="h-16 max-w-40 rounded-md border border-emerald-100 bg-white p-2 object-contain" /> : <div className="flex h-14 w-14 items-center justify-center rounded-md bg-white text-emerald-700"><BriefcaseBusiness className="h-7 w-7" /></div>}<h1 className="mt-8 text-3xl font-bold">Portal Staf</h1><p className="mt-3 max-w-xs text-sm leading-6 text-slate-600">Kelola tugas, jadwal, absensi, izin, dan laporan operasional sekolah dalam satu ruang kerja.</p></div><p className="text-xs text-slate-500">{appName || "Sistem Informasi Sekolah"}</p></section>
        <section className="min-w-0 p-6 sm:p-10">
          ...
          <form onSubmit={handleLogin} className="mt-8 min-w-0 space-y-5">
            ...
            <button disabled={isLoading || !identifier.trim() || !password} className="flex h-12 w-full items-center justify-center gap-2 rounded-md bg-emerald-700 px-4 text-sm font-bold text-white hover:bg-emerald-800 disabled:cursor-not-allowed disabled:bg-slate-200 disabled:text-slate-600 disabled:opacity-100">{isLoading ? "Memeriksa akun..." : "Masuk ke Portal"}{!isLoading && <ArrowRight className="h-4 w-4" />}</button>
          </form>
