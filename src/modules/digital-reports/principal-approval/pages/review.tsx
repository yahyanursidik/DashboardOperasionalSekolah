...
import { ArrowLeft, Loader2, AlertTriangle, History, Award, Edit3 } from "lucide-react";
...
export const PrincipalApprovalForm: React.FC = () => {
  ...
  const [principalNote, setPrincipalNote] = useState({ id: "", text: "" });
  ...
  useEffect(() => {
    if (notesData?.data) {
      ...
      if (prNote) setPrincipalNote({ id: prNote.id as string, text: prNote.note });
    }
  }, [notesData?.data]);

  const handleSavePrincipalNote = async (text: string) => {
    if (!user?.id) return;
    try {
      if (principalNote.id) {
        await supabaseClient.from('student_report_notes').update({ note: text, updated_by: user.id }).eq('id', principalNote.id);
      } else if (text.trim()) {
        const { data } = await supabaseClient.from('student_report_notes').insert({
          report_id: id,
          note_type: 'principal_note',
          note: text,
          parent_visible: true,
          created_by: user.id,
          updated_by: user.id
        }).select('id').single();
        if (data) setPrincipalNote({ id: (data as any).id, text });
      }
      ...
      await logAudit(
        ...
        principalNote.id || 'new',
        null,
        { note_type: 'principal_note', message: 'Principal saved note' }
      );

      toast.success("Catatan Kepala Sekolah disimpan.");
    } catch (err) {
  ...
  const handleUpdateStatus = async (newStatus: string, actionName: string, comments?: string) => {
    ...
    try {
      await supabaseClient.from('report_reviews').insert({
        ...
        status_to: newStatus,
        comments: comments || `Principal Action: ${actionName}`
      });
      ...
      if (newStatus === 'revision_needed' && comments) {
        await supabaseClient.from('student_report_notes').insert({
          ...
          note: `[Dari Kepala Sekolah] ${comments}`,
    ...
    <div className="space-y-6 pb-24">
      ...
      <div className="bg-card border rounded-xl shadow-sm p-4 sticky top-16 z-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          <button onClick={() => navigate("/reports/principal-approval")} className="p-2 hover:bg-muted rounded-full transition-colors shrink-0">
        ...
        <div className="flex items-center gap-2">
          {reportData.status === 'principal_approval' && (
            <>
              <button 
              ...
              <button 
                onClick={() => handleUpdateStatus('approved', 'Approve Final')}
              ...
              >
                {isSubmitting ? <Loader2 className="w-4 h-4 animate-spin" /> : <Award className="w-4 h-4" />} Setujui Final
              </button>
            </>
          )}
        </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="space-y-6">
          ...
          {/* Principal Note Area */}
          <div className="bg-indigo-50 rounded-xl border border-indigo-200 shadow-sm overflow-hidden">
            <div className="px-5 py-3 border-b border-indigo-200 bg-indigo-100/50 font-bold text-indigo-900 flex items-center justify-between">
              <div className="flex items-center gap-2"><Edit3 className="w-4 h-4" /> Catatan Kepala Sekolah (Opsional)</div>
            </div>
            <div className="p-5">
               <textarea 
                rows={4}
                value={principalNote.text}
                onChange={(e) => setPrincipalNote(prev => ({ ...prev, text: e.target.value }))}
                onBlur={(e) => handleSavePrincipalNote(e.target.value)}
                placeholder="Tambahkan catatan khusus dari Kepala Sekolah untuk siswa ini jika diperlukan..."
                className="w-full px-3 py-2 border rounded-md focus:ring-2 focus:ring-indigo-500 text-sm resize-y"
              />
              <p className="text-xs text-indigo-700/70 mt-2">Disimpan otomatis saat Anda berpindah kursor.</p>
            </div>
          </div>
      ...
      {/* Revision Modal */}
      <Modal isOpen={isRevisionModalOpen} title="Kembalikan untuk Revisi" onClose={() => setIsRevisionModalOpen(false)}>
        <div className="p-6 space-y-4">
          <div className="bg-red-50 text-red-800 p-3 rounded-md text-sm border border-red-200 flex items-start gap-2">
            <AlertTriangle className="w-4 h-4 shrink-0 mt-0.5" />
            Rapor ini akan dikembalikan dengan status <strong>"Butuh Revisi"</strong>.
          </div>
          <div className="space-y-2">
            <label className="text-sm font-bold">Catatan Revisi dari Kepala Sekolah</label>
            <textarea 
              rows={4}
              value={revisionText}
              onChange={(e) => setRevisionText(e.target.value)}
              placeholder="Berikan instruksi apa yang perlu diperbaiki..."
