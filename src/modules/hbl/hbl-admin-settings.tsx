...
const PRESCHOOL_LEVELS = [{ value: "kb", label: "KB · Kelompok Bermain" }, { value: "tka", label: "TK-A" }, { value: "tkb", label: "TK-B" }];
const normalizePreschoolLevel = (value?: string | null) => {
  const text = String(value || "").toLowerCase();
  if (/(^|[^a-z])kb([^a-z]|$)|kelompok[ _-]*bermain/.test(text)) return "kb";
  if (/tk[ _-]*a|tka/.test(text)) return "tka";
  if (/tk[ _-]*b|tkb/.test(text)) return "tkb";
  return "";
};
...
export const HblAdminSettings: React.FC = () => {
  ...
  const [programForm, setProgramForm] = useState({ name: "", description: "", unit_id: activeUnitId || "", semester_id: activeSemesterId || "", preschool_level: "" });
  ...
  const loadBase = useCallback(async () => {
    ...
    const [programResult, unitResult, studentResult, semesterResult] = await Promise.all([
      ...
      hblDb.from("students").select("id,full_name,nis,unit_id,classes(name,level,grade_level)").eq("status", "active").order("full_name"),
  ...
  const createProgram = async (event: React.FormEvent) => {
    ...
    if (!programForm.unit_id || !selectedSemester || !programForm.preschool_level || programForm.name.trim().length < 3) return toast.error("Nama, jenjang KB/TK-A/TK-B, unit, dan semester program wajib diisi.");
    setIsSaving(true);
    const { data, error } = await hblDb.from("hbl_programs").insert({
      ...
      unit_id: programForm.unit_id, academic_year_id: selectedSemester.academic_year_id || activeYearId || null, semester_id: selectedSemester.id, preschool_level: programForm.preschool_level,
    }).select("id").single();
    setIsSaving(false);
    if (error) return toast.error("Program gagal dibuat", { description: error.message });
    toast.success("Program HBL dibuat.");
    setProgramForm((current) => ({ ...current, name: "", description: "", preschool_level: "" }));
  ...
  const candidateStudents = students.filter((student) => {
    if (selectedProgram?.unit_id && student.unit_id !== selectedProgram.unit_id) return false;
    return !selectedProgram?.preschool_level || normalizePreschoolLevel(student.classes?.level || student.classes?.name) === selectedProgram.preschool_level;
  });
    ...
    <section className="grid gap-5 xl:grid-cols-[340px_minmax(0,1fr)]">
      <div className="space-y-4">
        <form onSubmit={createProgram} className="space-y-3 rounded-xl border bg-card p-4">
          ...
          <label className="block text-xs font-semibold text-muted-foreground">Jenjang HBL<select aria-label="Jenjang HBL" value={programForm.preschool_level} onChange={(event) => setProgramForm({ ...programForm, preschool_level: event.target.value })} className="mt-1 h-10 w-full rounded-md border bg-background px-3 text-sm text-foreground" required><option value="">Pilih KB, TK-A, atau TK-B</option>{PRESCHOOL_LEVELS.map((level) => <option key={level.value} value={level.value}>{level.label}</option>)}</select></label>
        ...
        <div className="overflow-hidden rounded-xl border bg-card">
          ...
          <div className="divide-y">{visiblePrograms.map((program) => <button key={program.id} onClick={() => setSelectedProgramId(program.id)} className={`flex w-full items-center justify-between gap-3 p-4 text-left ${selectedProgramId === program.id ? "bg-primary/10" : "hover:bg-muted/40"}`}><span><span className="block text-sm font-bold">{program.name}</span><span className="mt-1 block text-xs text-muted-foreground">{PRESCHOOL_LEVELS.find((level) => level.value === program.preschool_level)?.label || "Jenjang belum diklasifikasikan"} · {program.units?.name} · {program.academic_years?.name || "Tahun ajaran"} · Semester {program.semesters?.name || "-"} · {program.status === "published" ? "Terbit" : program.status === "archived" ? "Arsip" : "Draf"}</span></span><ChevronRight className="h-4 w-4 shrink-0" /></button>)}{visiblePrograms.length === 0 && <p className="p-6 text-center text-sm text-muted-foreground">Belum ada program pada semester ini.</p>}</div>
        </div>
      </div>
      ...
      {!selectedProgram ? <div className="flex min-h-80 items-center justify-center rounded-xl border border-dashed text-sm text-muted-foreground">Buat atau pilih program untuk mulai menyusun LMS.</div> : <div className="space-y-5">
        ...
        <section className="rounded-xl border bg-card p-5"><div className="mb-4 flex items-center gap-2"><Users className="h-5 w-5 text-primary" /><div><h3 className="font-bold">Peserta program</h3><p className="text-xs text-muted-foreground">Siswa ditampilkan hanya bila unit dan jenjang kelasnya sesuai program.</p></div></div><div className="grid gap-4 lg:grid-cols-2"><div className="max-h-64 overflow-y-auto rounded-lg border divide-y">{candidateStudents.map((student) => <label key={student.id} className="flex cursor-pointer items-center gap-3 p-3 text-sm hover:bg-muted/30"><input type="checkbox" checked={selectedStudentIds.has(student.id)} onChange={() => setSelectedStudentIds((current) => { const next = new Set(current); next.has(student.id) ? next.delete(student.id) : next.add(student.id); return next; })} /><span><span className="block font-semibold">{student.full_name}</span><span className="text-xs text-muted-foreground">{student.nis || "Tanpa NIS"} · {student.classes?.name || "Belum ada kelas"}</span></span></label>)}{candidateStudents.length === 0 && <p className="p-5 text-center text-sm text-muted-foreground">{selectedProgram?.preschool_level ? "Tidak ada siswa dengan jenjang kelas yang sesuai. Lengkapi level kelas sebagai KB, TK-A, atau TK-B terlebih dahulu." : "Belum ada siswa aktif pada unit program."}</p>}</div><div><button onClick={() => void saveEnrollments()} disabled={selectedStudentIds.size === 0} className="mb-3 inline-flex h-10 w-full items-center justify-center gap-2 rounded-md bg-primary px-4 text-sm font-bold text-primary-foreground disabled:opacity-50"><Users className="h-4 w-4" /> Tambahkan {selectedStudentIds.size || ""} Siswa</button><div className="max-h-52 space-y-2 overflow-y-auto">{enrollments.map((item) => <div key={item.id} className="flex items-center justify-between gap-3 rounded-md border p-2.5"><span className="text-sm"><strong>{item.students?.full_name}</strong><span className="block text-xs text-muted-foreground">{item.students?.classes?.name || "Belum ada kelas"}</span></span><button onClick={() => void removeEnrollment(item.id)} className="p-1.5 text-red-600" title="Keluarkan peserta"><Trash2 className="h-4 w-4" /></button></div>)}{enrollments.length === 0 && <p className="text-center text-sm text-muted-foreground">Belum ada peserta.</p>}</div></div></div></section>
