...
export const TahsinHalaqohsList: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Data Halaqoh Tahsin"
        description="Kelola kelompok halaqoh tahsin / tilawah dan guru pengampunya."
      />
          ...
          <table className="w-full text-sm text-left">
            <thead className="bg-muted/50 border-b">
              <tr>
                ...
                <th className="px-6 py-3 font-semibold text-muted-foreground">Guru Tahsin</th>
            ...
            <tbody className="divide-y">
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={4} className="px-6 py-8 text-center text-muted-foreground">
                    Belum ada data halaqoh tahsin pada semester ini.
                  </td>
                </tr>
              ) : (
                records.map((record) => (
                  <tr key={record.id} className="hover:bg-muted/30 transition-colors">
                    ...
                    <td className="px-6 py-4 font-medium">{record.employees?.full_name || "-"}</td>
                    <td className="px-6 py-4 text-muted-foreground">{record.description || "-"}</td>
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <Link
                          to={`/tahsin-halaqohs/show/${record.id}`}
                        ...
                        <Link
                          to={`/tahsin-halaqohs/edit/${record.id}`}
