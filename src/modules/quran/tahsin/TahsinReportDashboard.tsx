...
export const TahsinReportDashboard: React.FC = () => {
  ...
  const halaqohIdSet = useMemo(() => new Set(halaqohs.map((halaqoh: any) => halaqoh.id)), [halaqohs]);

  const { data: membersData } = useList({
    resource: "tahfidz_halaqoh_members",
    meta: { select: "id, halaqoh_id, student_id, students(id, full_name, nis, class_id, classes(name, unit_id, units(name)))" },
    pagination: { mode: "off" },
  });
  const allMembers = useMemo(() => {
    return (membersData?.data || []).filter((member: any) => halaqohIdSet.has(member.halaqoh_id));
  }, [halaqohIdSet, membersData?.data]);
  const selectedHalaqohMembers = useMemo(() => {
    return selectedHalaqohId ? allMembers.filter((member: any) => member.halaqoh_id === selectedHalaqohId) : [];
  }, [allMembers, selectedHalaqohId]);
  const selectedHalaqohStudentIds = useMemo(() => new Set(selectedHalaqohMembers.map((member: any) => member.student_id)), [selectedHalaqohMembers]);
  ...
  const studentPool = useMemo(() => {
    const source = selectedHalaqohId
      ? selectedHalaqohMembers.map((member: any) => member.students).filter(Boolean)
      : studentsData?.data || [];
  ...
  }, [search, selectedClassId, selectedHalaqohId, selectedHalaqohMembers, selectedUnitId, studentsData?.data]);
  ...
  const targets = useMemo(() => {
    return (targetsData?.data || []).filter((target: any) => {
      ...
      if (selectedHalaqohId) return selectedHalaqohStudentIds.has(target.student_id);
      return visibleStudentIds.size === 0 || visibleStudentIds.has(target.student_id);
    });
  }, [selectedHalaqohId, selectedHalaqohStudentIds, selectedStudentId, targetsData?.data, visibleStudentIds]);

  const records = useMemo(() => {
    return (recordsData?.data || []).filter((record: any) => {
      if (selectedStudentId) return true;
      if (selectedHalaqohId) return selectedHalaqohStudentIds.has(record.student_id) || record.halaqoh_id === selectedHalaqohId;
      return visibleStudentIds.size === 0 || visibleStudentIds.has(record.student_id);
    });
  }, [recordsData?.data, selectedHalaqohId, selectedHalaqohStudentIds, selectedStudentId, visibleStudentIds]);

  const assessments = useMemo(() => {
    return (assessmentsData?.data || []).filter((assessment: any) => {
      if (selectedStudentId) return true;
      if (selectedHalaqohId) return selectedHalaqohStudentIds.has(assessment.student_id);
      return visibleStudentIds.size === 0 || visibleStudentIds.has(assessment.student_id);
    });
  }, [assessmentsData?.data, selectedHalaqohId, selectedHalaqohStudentIds, selectedStudentId, visibleStudentIds]);
  ...
  const selectedStudentHalaqohId = useMemo(() => {
    if (selectedHalaqohId) return selectedHalaqohId;
    return allMembers.find((member: any) => member.student_id === selectedStudentId)?.halaqoh_id || "";
  }, [allMembers, selectedHalaqohId, selectedStudentId]);
  ...
  const studentRows = useMemo(() => {
    ...
    records.forEach((record: any) => {
      ...
      if (!row) return;
      row.recordCount += 1;
      if (record.fluency_score === "Sangat Lancar" || record.fluency_score === "Lancar") row.fluentRecords += 1;
      if (record.fluency_score === "Mengulang") row.repeatRecords += 1;
      if (record.tajwid_score || record.makhroj_score) row.qualityRecords += 1;
    ...
    assessments.forEach((assessment: any) => {
      ...
      if (assessment.status === "Lulus Bersyarat") row.conditionalAssessments += 1;
  ...
  const fluentRecords = records.filter((record: any) => record.fluency_score === "Sangat Lancar" || record.fluency_score === "Lancar").length;
  const repeatRecords = records.filter((record: any) => record.fluency_score === "Mengulang").length;
  const qualityFilledRecords = records.filter((record: any) => record.tajwid_score || record.makhroj_score).length;
  const passedAssessments = assessments.filter((assessment: any) => assessment.status === "Lulus").length;
  const conditionalAssessments = assessments.filter((assessment: any) => assessment.status === "Lulus Bersyarat").length;
  const qualityScore = Math.round(
    (totalTargets ? (completedTargets / totalTargets) * 25 : 0) +
    (totalStudents ? (studentRows.filter((row) => row.recordCount > 0).length / totalStudents) * 25 : 0) +
    (totalRecords ? (fluentRecords / totalRecords) * 25 : 0) +
    (assessments.length ? (passedAssessments / assessments.length) * 25 : 0)
  );
  ...
  const chartData = useMemo(() => {
    const grouped = new Map<string, { label: string; jurnal: number; ujian: number }>();
    [...records].reverse().forEach((record: any) => {
      ...
      if (!grouped.has(key)) grouped.set(key, { label, jurnal: 0, ujian: 0 });
      grouped.get(key)!.jurnal += 1;
    });
  ...
  const halaqohChartData = useMemo(() => {
    const grouped = new Map<string, { id: string; name: string; anggota: number; jurnal: number; ujian: number }>();
    halaqohs.forEach((halaqoh: any) => grouped.set(halaqoh.id, { id: halaqoh.id, name: halaqoh.name, anggota: 0, jurnal: 0, ujian: 0 }));
    allMembers.forEach((member: any) => {
      if (!grouped.has(member.halaqoh_id)) grouped.set(member.halaqoh_id, { id: member.halaqoh_id, name: "Halaqoh", anggota: 0, jurnal: 0, ujian: 0 });
      grouped.get(member.halaqoh_id)!.anggota += 1;
    });
    records.forEach((record: any) => {
      if (!record.halaqoh_id) return;
      if (!grouped.has(record.halaqoh_id)) grouped.set(record.halaqoh_id, { id: record.halaqoh_id, name: record.tahfidz_halaqohs?.name || "Halaqoh", anggota: 0, jurnal: 0, ujian: 0 });
      grouped.get(record.halaqoh_id)!.jurnal += 1;
    });
    assessments.forEach((assessment: any) => {
      const member = allMembers.find((item: any) => item.student_id === assessment.student_id);
      if (!member) return;
      if (!grouped.has(member.halaqoh_id)) grouped.set(member.halaqoh_id, { id: member.halaqoh_id, name: "Halaqoh", anggota: 0, jurnal: 0, ujian: 0 });
      grouped.get(member.halaqoh_id)!.ujian += 1;
    });
    return Array.from(grouped.values())
      .filter((item) => (!selectedHalaqohId || item.id === selectedHalaqohId) && (item.anggota || item.jurnal || item.ujian))
      .slice(0, 8);
  }, [allMembers, assessments, halaqohs, records, selectedHalaqohId]);
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Laporan Mutu Tahsin"
        description="Pantau target, jurnal harian, kualitas bacaan, ujian kenaikan jilid, dan tindak lanjut Tahsin."
      />
        ...
        <div className="grid gap-5 lg:grid-cols-[1fr_1.3fr]">
          <div>
            <div className="inline-flex items-center gap-2 rounded-md bg-emerald-50 px-3 py-1 text-sm font-semibold text-emerald-700">
              ...
              Laporan tahsin berbasis mutu
            </div>
            <h2 className="mt-3 text-2xl font-bold">Dari target bacaan sampai kenaikan jilid dalam satu monitoring</h2>
            <p className="mt-2 text-sm leading-6 text-muted-foreground">
              Halaman ini membantu guru dan pimpinan membaca konsistensi jurnal, mutu tajwid-makhraj, status ujian, dan siswa yang perlu pembinaan ulang.
            </p>
          </div>
        ...
        <div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
          <div className="flex flex-1 flex-wrap gap-2">
            ...
            >
              ...
              {studentPool.map((student: any) => <option key={student.id} value={student.id}>{student.full_name}</option>)}
            </select>
          </div>
          <button
            type="button"
      ...
      <section className="grid gap-4 lg:grid-cols-3">
        ...
        <div className="rounded-xl border bg-card p-5 shadow-sm lg:col-span-2">
          <div className="flex items-center gap-2 text-sm font-semibold text-muted-foreground">
            ...
            Definition of done laporan tahsin
          </div>
          <div className="mt-4 grid gap-2 md:grid-cols-5">
            {["Target aktif", "Jurnal berjalan", "Tajwid-makhraj", "Ujian jilid", "Tindak lanjut"].map((item) => (
      ...
      <section className="grid gap-6 xl:grid-cols-[1.2fr_0.8fr]">
        <div className="rounded-xl border bg-card p-5 shadow-sm">
          ...
          {chartData.length ? (
            <div className="h-[300px]">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={chartData} margin={{ top: 8, right: 20, bottom: 8, left: 0 }}>
                  ...
                  <Tooltip />
                  <Legend />
                  <Line type="monotone" dataKey="jurnal" name="Jurnal" stroke="#059669" strokeWidth={3} dot={{ r: 3 }} />
            ...
            <div className="flex h-[220px] items-center justify-center rounded-lg border border-dashed text-sm text-muted-foreground">
              Belum ada data jurnal atau ujian pada filter ini.
            </div>
          )}
        </div>
        ...
        <div className="rounded-xl border bg-card p-5 shadow-sm">
          ...
          {halaqohChartData.length ? (
            <div className="h-[300px]">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={halaqohChartData} layout="vertical" margin={{ top: 8, right: 20, bottom: 8, left: 20 }}>
                  ...
                  <Tooltip />
                  <Bar dataKey="jurnal" name="Jurnal" radius={[0, 6, 6, 0]}>
            ...
            <div className="flex h-[220px] items-center justify-center rounded-lg border border-dashed text-sm text-muted-foreground">
              Lengkapi anggota halaqoh atau jurnal Tahsin.
            </div>
          )}
        </div>
      </section>
          ...
          <div className="rounded-xl border bg-card p-5 shadow-sm">
            ...
            <div className="mt-4 grid grid-cols-3 gap-2 text-center">
              ...
              <div className="rounded-lg border bg-background p-3">
                ...
                <p className="text-[11px] text-muted-foreground">Jurnal</p>
              </div>
              <div className="rounded-lg border bg-background p-3">
            ...
            <div className="mt-5 grid gap-2">
              <Link to={`/tahsin-records/create?student_id=${selectedStudentId}&halaqoh_id=${selectedStudentHalaqohId}`} className="flex items-center justify-between rounded-lg border bg-background px-3 py-2 text-sm font-semibold hover:bg-muted">
              ...
              <Link to={`/tahsin-student-targets/create?student_id=${selectedStudentId}&halaqoh_id=${selectedStudentHalaqohId}`} className="flex items-center justify-between rounded-lg border bg-background px-3 py-2 text-sm font-semibold hover:bg-muted">
              ...
              <Link to={`/tahsin-assessments/create?student_id=${selectedStudentId}&halaqoh_id=${selectedStudentHalaqohId}`} className="flex items-center justify-between rounded-lg border bg-background px-3 py-2 text-sm font-semibold hover:bg-muted">
                <span className="flex items-center gap-2"><Award className="h-4 w-4 text-emerald-600" /> Input ujian jilid</span>
          ...
          <div className="grid gap-6">
            <div className="rounded-xl border bg-card shadow-sm overflow-hidden">
              <div className="border-b bg-muted/20 px-5 py-4">
                <h3 className="font-semibold">Jurnal terakhir</h3>
              </div>
                  ...
                  <tbody className="divide-y">
                    {selectedStudentRecords.slice(0, 8).map((record: any) => (
                      <tr key={record.id}>
                        ...
                        <td className="px-5 py-3">
                          <span className={`rounded-md border px-2 py-1 text-xs font-semibold ${fluencyClass(record.fluency_score)}`}>{record.fluency_score}</span>
                        </td>
                      </tr>
                    ))}
                    {selectedStudentRecords.length === 0 && (
                      <tr><td colSpan={3} className="px-5 py-8 text-center text-muted-foreground">Belum ada jurnal Tahsin.</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
            ...
            <div className="rounded-xl border bg-card shadow-sm overflow-hidden">
              <div className="border-b bg-muted/20 px-5 py-4">
                <h3 className="font-semibold">Riwayat ujian kenaikan jilid</h3>
              </div>
              <div className="overflow-x-auto">
                <table className="w-full text-left text-sm">
                  <thead className="bg-muted/50">
                  ...
                  <tbody className="divide-y">
                    {selectedStudentAssessments.slice(0, 8).map((assessment: any) => (
                      <tr key={assessment.id}>
                        ...
                        <td className="px-5 py-3">
                          ...
                          <span className={`inline-flex rounded-md border px-2 py-1 text-xs font-semibold ${assessmentClass(assessment.status)}`}>
                    ...
                    {selectedStudentAssessments.length === 0 && (
                      <tr><td colSpan={3} className="px-5 py-8 text-center text-muted-foreground">Belum ada ujian kenaikan jilid.</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
      ...
      <section className="rounded-xl border bg-card shadow-sm overflow-hidden">
        <div className="flex flex-col gap-3 border-b bg-muted/20 px-5 py-4 md:flex-row md:items-center md:justify-between">
          <div>
            <h3 className="font-semibold">Rekap siswa tahsin</h3>
          ...
          <div className="flex items-center gap-2 text-sm">
            ...
            <select value={pageSize} onChange={(event) => setPageSize(Number(event.target.value))} className="h-9 rounded-md border bg-background px-2 text-sm">
          ...
          <table className="w-full text-left text-sm">
            <thead className="bg-muted/50">
              <tr>
                ...
                <th className="px-5 py-3 font-semibold text-muted-foreground">Jurnal</th>
                <th className="px-5 py-3 font-semibold text-muted-foreground">Ujian</th>
            ...
            <tbody className="divide-y">
              {paginatedStudentRows.map((row) => {
                const rowHalaqohId = selectedHalaqohId || allMembers.find((member: any) => member.student_id === row.id)?.halaqoh_id || "";
                return (
                  <tr key={row.id} className="hover:bg-muted/30">
                    ...
                    <td className="px-5 py-3">
                      ...
                      <p className="text-xs text-muted-foreground">{row.qualityRecords} skor tajwid/makhraj</p>
                      {row.repeatRecords > 0 && <p className="text-xs text-red-600">{row.repeatRecords} mengulang</p>}
                    </td>
                    <td className="px-5 py-3">
                      <p>{row.passedAssessments}/{row.assessmentCount}</p>
                      {row.conditionalAssessments > 0 && <p className="text-xs text-amber-600">{row.conditionalAssessments} bersyarat</p>}
                    </td>
                    ...
                    <td className="px-5 py-3 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <Link to={`/tahsin-records?student_id=${row.id}`} className="rounded-lg p-2 text-emerald-600 hover:bg-emerald-50" title="Jurnal tahsin">
                          <BookOpen className="h-4 w-4" />
                        </Link>
                        <Link to={`/tahsin-student-targets?student_id=${row.id}`} className="rounded-lg p-2 text-cyan-600 hover:bg-cyan-50" title="Target tahsin">
                          <Target className="h-4 w-4" />
                        </Link>
                        <Link to={`/tahsin-assessments?student_id=${row.id}&halaqoh_id=${rowHalaqohId}`} className="rounded-lg p-2 text-purple-600 hover:bg-purple-50" title="Ujian jilid">
        ...
        {totalStudentRows > 0 && (
          <div className="flex flex-col gap-3 border-t bg-muted/10 px-5 py-4 md:flex-row md:items-center md:justify-between">
            ...
            <div className="flex items-center gap-2">
              <button type="button" onClick={() => setCurrentPage(1)} disabled={currentPage === 1} className="rounded-md border bg-background px-3 py-1.5 text-sm font-medium transition-colors hover:bg-muted disabled:cursor-not-allowed disabled:opacity-50">
                Awal
              </button>
              <button type="button" onClick={() => setCurrentPage((page) => Math.max(1, page - 1))} disabled={currentPage === 1} className="rounded-md border bg-background px-3 py-1.5 text-sm font-medium transition-colors hover:bg-muted disabled:cursor-not-allowed disabled:opacity-50">
                Sebelumnya
              </button>
              <span className="rounded-md border bg-background px-3 py-1.5 text-sm font-semibold">{currentPage}</span>
              <button type="button" onClick={() => setCurrentPage((page) => Math.min(pageCount, page + 1))} disabled={currentPage === pageCount} className="rounded-md border bg-background px-3 py-1.5 text-sm font-medium transition-colors hover:bg-muted disabled:cursor-not-allowed disabled:opacity-50">
                Berikutnya
              </button>
              <button type="button" onClick={() => setCurrentPage(pageCount)} disabled={currentPage === pageCount} className="rounded-md border bg-background px-3 py-1.5 text-sm font-medium transition-colors hover:bg-muted disabled:cursor-not-allowed disabled:opacity-50">
                Akhir
              </button>
            </div>
          </div>
