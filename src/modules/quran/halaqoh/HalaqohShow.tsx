...
export const HalaqohShow: React.FC = () => {
  ...
  const { data: membersData, isLoading: isLoadingMembers, refetch: refetchMembers } = useList({
    ...
    meta: { select: "*, students(id, name, nis, nisn, classes(name))" },
    pagination: { mode: "off" }
  });
  ...
  const { data: allStudents } = useList({
    ...
    meta: { select: "id, name, classes(name)" },
    pagination: { mode: "off" }
  });
  ...
  const handleRemoveMember = (memberId: string) => {
    if (window.confirm("Keluarkan santri dari halaqoh ini?")) {
      deleteMutate({
        resource: "tahfidz_halaqoh_members",
        id: memberId,
      }, {
          ...
          <div className="bg-card border rounded-xl shadow-sm p-6 space-y-4">
            ...
            <div className="space-y-3 text-sm">
              ...
              <div>
                ...
                <p className="font-medium">{halaqoh?.employees?.name || "-"}</p>
              </div>
              <div>
                  ...
                  >
                    ...
                    {availableStudents.map(student => (
                      <option key={student.id} value={student.id}>
                        {student.name} ({student.classes?.name || "Tanpa Kelas"})
                      </option>
                    ))}
                  </select>
                  <button
                  ...
                  {isLoadingMembers ? (
                    ...
                    <tr>
                      <td colSpan={4} className="px-6 py-12 text-center">
                        ...
                        <p className="text-muted-foreground font-medium">Belum ada anggota di halaqoh ini</p>
                    ...
                    members.map((member) => (
                      <tr key={member.id} className="hover:bg-muted/30 transition-colors">
                        <td className="px-6 py-4 font-bold text-gray-900">{member.students?.name}</td>
