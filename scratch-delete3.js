...
async function forceDelete() {
  ...
  while (!success && attempts < 20) {
    ...
    // Parse table name from error: ...violates foreign key constraint "..." on table "target_table"
    const match = error.message.match(/on table "([^"]+)"/);
    if (match && match[1]) {
      const table = match[1];
      console.log(`Cleaning up referencing table: ${table}`);
      // Usually the foreign key is unit_id
      ...
      if (delErr) {
        ...
        break; // Stop to prevent infinite loop on unexpected error
      } else {
